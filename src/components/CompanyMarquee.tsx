    ...
    <div className="relative">
      ...
      >
        {[...companies, ...companies].map((company, i) => (
          <div
            key={`${company.name}-${i}`}
            className="glass-card px-6 py-4 flex items-center justify-center min-w-[140px] gap-3 hover:border-primary/30 transition-colors"
          >
            <img
              src={company.logo}
              alt={company.name}
            ...
            />
            <span className="text-muted-foreground font-display font-semibold text-sm tracking-wide">
              {company.name}
            </span>
          </div>
        ))}
